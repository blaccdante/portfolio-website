    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
      {repos.map((repo) => (
        <a
          ...
          className="group rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300 p-4 sm:p-5 hover:scale-105 hover:shadow-lg hover:shadow-white/10 touch-manipulation"
        >
          <div className="flex items-start justify-between mb-2">
            <h3 className="text-white font-semibold text-base sm:text-lg group-hover:text-gray-300 transition-colors line-clamp-2 flex-1 mr-2">
              {repo.name}
            </h3>
          ...
          {repo.topics && repo.topics.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {repo.topics.slice(0, 4).map((t) => (
                <span key={t} className="text-[10px] sm:text-[11px] px-2 py-1 rounded-full bg-gradient-to-r from-gray-600/20 to-white/20 text-white/70 border border-white/10">
                  {t}
                </span>
              ))}
            </div>
